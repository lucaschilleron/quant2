...
linear2 <- lm(hsgrad ~ small + race_f + yearssmall, data = df)

logit2 <- glm(hsgrad ~ small + race_f + yearssmall,
              family = binomial,
              data = df)
...
p1 <- plot_predictions(logit2,
                       condition = c("yearssmall", "small"))

ggsave("predicted_probabilities_yearssmall.png",
       plot = p1,
       width = 7,
       height = 5)
