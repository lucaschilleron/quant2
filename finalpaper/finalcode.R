...
slopes_m1 <- avg_slopes(
  lpm_full,
  variables = c("lag_pol", "lag_cap", "lag_gdppc", "lag_conflict", "lag_regime", "log_years"),
  by = "support_collapsed"
) %>%
  mutate(model_name = "Model 1: v2cacamps (Raw Pol)")

slopes_m2 <- avg_slopes(
  lpm_full2,
  variables = c("lag_pol2", "lag_cap", "lag_gdppc", "lag_conflict", "lag_regime", "log_years"),
  by = "support_collapsed"
) %>%
  mutate(model_name = "Model 2: v2smpolsoc (Alt Pol)")

plot_data_sig <- bind_rows(slopes_m1, slopes_m2) %>%
  mutate(
    facet_term = case_when(
      term %in% c("lag_pol", "lag_pol2") ~ "Political Polarization",
      term == "lag_cap"                  ~ "Fiscal Capacity",
      term == "lag_gdppc"                ~ "Log GDP per Capita",
      term == "lag_conflict"             ~ "Political Violence",
      term == "lag_regime"               ~ "Democracy Index",
      term == "log_years"                ~ "Log Years in Power",
      TRUE                               ~ term
    ),
    support_collapsed = factor(support_collapsed, levels = rev(levels(support_collapsed))),
    
    is_significant = if_else(conf.low > 0 | conf.high < 0, "Significant", "Not Significant")
  )

fullplot <- ggplot(plot_data_sig, aes(
  x = estimate, 
  y = support_collapsed, 
  xmin = conf.low, 
  xmax = conf.high, 
  ...
  geom_vline(xintercept = 0, color = "grey50", linetype = "dashed", alpha = 0.8) +
  
  geom_pointrange(position = position_dodge(width = 0.6), size = 0.5) +
  
  facet_wrap(~ facet_term, scales = "fixed", ncol = 2) +
  
  scale_color_manual(values = c("Model 1: v2cacamps (Raw Pol)" = "#4B0082", 
                                "Model 2: v2smpolsoc (Alt Pol)" = "darkseagreen4")) +
  ...
  scale_shape_manual(values = c("Significant" = 16, "Not Significant" = 1)) +
    labs(
    title = "Comparative Analysis of Regime Instability Across Models",
    subtitle = "Solid points represent absolute slopes significant at the 95% level; faded open points are non-significant.",
    x = "Absolute Change in Probability (1 SD increase in predictor)",
    y = "Current Regime Support Coalition",
    color = "Model Specification",
    alpha = "Statistical Clarity", 
    shape = "Statistical Clarity"
  ) +
  theme_minimal() +
  theme(
  ...
  guides(
    alpha = guide_legend(title = "Significance (95% CI excludes 0)"),
    shape = guide_legend(title = "Significance (95% CI excludes 0)"),
    color = guide_legend(title = "Model Specification")
  )
...
estimates_m1 <- modelsummary::get_estimates(lpm_full) %>%
  mutate(model_name = "Model 1: v2cacamps (Raw Pol)")

estimates_m2 <- modelsummary::get_estimates(lpm_full2) %>%
  mutate(model_name = "Model 2: v2smpolsoc (Alt Pol)")

interaction_data <- bind_rows(estimates_m1, estimates_m2) %>%
  filter(str_detect(term, ":")) %>%
  mutate(
    coalition = str_extract(term, "(?<=support_collapsed)[^:]+"),
    covariate_raw = str_extract(term, "(?<=:).*"),
    
    facet_term = case_when(
      covariate_raw %in% c("lag_pol", "lag_pol2") ~ "Political Polarization",
      covariate_raw == "lag_cap"                  ~ "Fiscal Capacity",
      covariate_raw == "lag_gdppc"                ~ "Log GDP per Capita",
      covariate_raw == "lag_conflict"             ~ "Political Violence",
      covariate_raw == "lag_regime"               ~ "Democracy Index",
      covariate_raw == "log_years"                ~ "Log Years in Power",
      TRUE                                        ~ covariate_raw
    ),
    
    is_sig_interaction = if_else(p.value < 0.05, "Differs from Elite", "Same as Elite"),
    
    coalition = factor(coalition, levels = rev(c("Identity", "Military", "Bureaucratic/Political", 
                                                 "Popular/Mass", "External", "Other")))
  )

interactionplot <- ggplot(interaction_data, aes(
  x = estimate, 
  y = coalition, 
  xmin = estimate - (1.96 * std.error), # Manually build 95% CI bounds
  xmax = estimate + (1.96 * std.error), 
  color = model_name,
  alpha = is_sig_interaction,
  shape = is_sig_interaction
)) +
  geom_vline(xintercept = 0, color = "darkred", linetype = "solid", alpha = 0.4) +
  ...
  facet_wrap(~ facet_term, scales = "fixed", ncol = 2) +
    ...
    scale_color_manual(values = c("Model 1: v2cacamps (Raw Pol)" = "#4B0082", 
                                "Model 2: v2smpolsoc (Alt Pol)" = "darkseagreen4")) +
  scale_alpha_manual(values = c("Differs from Elite" = 1, "Same as Elite" = 0.35)) +
  scale_shape_manual(values = c("Differs from Elite" = 16, "Same as Elite" = 1)) +
  
  labs(
    title = "Significance of Interaction Terms (Slope Differences vs. Elite)",
    subtitle = "Solid points indicate the coalition's slope is statistically different from the Elite baseline slope (p < 0.05).",
    x = "Difference in Coefficient Slope relative to Elite Baseline",
    y = "Comparison Support Coalition",
    color = "Model Specification"
  ) +
  theme_minimal() +
  theme(
  ...
  guides(
    alpha = guide_legend(title = "Interaction Effect (p < 0.05)"),
    shape = guide_legend(title = "Interaction Effect (p < 0.05)"),
    color = guide_legend(title = "Model Specification")
  )
...
tidy_cr_models <- function(model_list, spec_name) {
  bind_rows(lapply(names(model_list), function(dest) {
    sum_mod   <- summary(model_list[[dest]])
    coef_mat  <- sum_mod$coef  # Matrix columns: [1]coef, [2]exp(coef), [3]se.coef, [4]z, [5]p-value
    ...
    data.frame(
      ...
      mutate(
        shr      = exp(coef),
        shr_low  = exp(coef - 1.96 * se_coef),
        shr_high = exp(coef + 1.96 * se_coef),
        sig      = case_when(
          p_val < 0.01 ~ "p < 0.01",
          p_val < 0.05 ~ "p < 0.05",
          p_val < 0.10 ~ "p < 0.10",
...
# --- PROCESS, REVERSE ORDER, AND COMBINE ---
